...
class DP_Webhook
{
    ...
    public function process()
    {
        ...
        {
            if (isset($_SERVER['HTTP_X_DURIANPAY_SIGNATURE']) === true)
            {
                $durianpayWebhookSecret = $this->durianpay->getSetting('webhook_secret');

                //
                // If the webhook secret isn't set on wordpress, return
                //
                if (empty($durianpayWebhookSecret) === true)
                {
                    return;
                }
                ...
                try
                {
                    $this->api->utility->verifyWebhookSignature($post,
                                                                $_SERVER['HTTP_X_DURIANPAY_SIGNATURE'],
                                                                $durianpayWebhookSecret);
                }
                catch (Errors\SignatureVerificationError $e)
                {
                    $log = array(
                        ...
                        'data'      => $data,
                        'event'     => 'durianpay.wc.signature.verify_failed'
                    );
    ...
    protected function paymentAuthorized(array $data)
    {
        ...
        $durianpayPaymentId = $data['payload']['payment']['entity']['id'];

        $payment = $this->getPaymentEntity($durianpayPaymentId, $data);
        ...
        if ($payment['status'] === 'captured')
        {
            $success = true;
                 ...
                 ($this->durianpay->getSetting('payment_action') === WC_Durianpay::CAPTURE))
        {
            //
        ...
        $this->durianpay->updateOrder($order, $success, $errorMessage, $durianpayPaymentId, null, true);
    ...
    protected function virtualAccountCredited(array $data)
    {
        ...
        $this->durianpay->updateOrder($order, $success, $errorMessage, $durianpayPaymentId, $virtualAccountId, true);

        // Graceful exit since payment is now processed.
        exit;
    }

    protected function getPaymentEntity($durianpayPaymentId, $data)
    {
        try
        {
            $payment = $this->api->payment->fetch($durianpayPaymentId);
        }
        catch (Exception $e)
        {
            $log = array(
    ...
    public function refundedCreated(array $data)
    {
        ...
        $durianpayPaymentId = $data['payload']['refund']['entity']['payment_id'];
