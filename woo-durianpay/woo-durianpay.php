<?php
/*
 * Plugin Name: Durianpay
 * Plugin URI: https://durianpay.com
 * Description: Durianpay Payment Gateway Integration for WooCommerce.Durianpay Welcome Back Offer: New to Durianpay
 * Version: 2.0.0
 * Stable tag: 2.0.0
 * Author: Team Durianpay
 * WC tested up to: 9.1.2
 * Author URI: https://durianpay.com
*/
...
function woocommerce_razorpay_init()
{
    ...
    class WC_Razorpay extends WC_Payment_Gateway
    {
        ...
        public function __construct($hooks = true)
        {
            ...
            // Load preference API call only for administrative interface + durianpay payment settings page.
        ...
        public function init_form_fields()
        {
            ...
            $defaultFormFields = array(
                ...
                'key_id' => array(
                    ...
                    'type' => 'text',
                    'description' => __('The api key can be generated from durianpay Dashboard. Use test or live for test or live mode.', $this->id)
                ),
                'expired_order' => array(
                    'title' => __('Expired order', $this->id),
                    'type' => 'text',
                    'description' => __('The Format can be dd:hh:mm.', $this->id)
                ),
                'webhook_url' => array(
                    'title' => __('webhook url', $this->id),
                    'type' => 'text',
                    'description' => __('The value for notification webhook', $this->id)
                ),
                ...
                try
                {
                        ...
                        $rtbActivationStatus['widget_enabled'] === true)
                    {
                        $this->form_fields['rtb_widget_title'] =  array(
                            'title' => '<span style="font-size: 20px;">' . __('Durianpay Trusted Business'). '</span>',
                            'type' => 'title'
                        );
        ...
        public function autoEnableWebhook()
        {
            ...
            catch (Exception $e)
            {
                ...
                ?>
                <div class="notice error is-dismissible" >
                    <p><b><?php _e( 'Please check Api key'); ?></b></p>
                </div>
                <?php
            ...
            if ($webhookExist)
            {
                ...
                rzpLogInfo('Updating durianoay webhook');
            ...
            else
            {
                ...
                rzpLogInfo('Creating durianpay webhook');
        ...
        public function pluginInstrumentation()
        {
            ...
            catch (\Razorpay\Api\Errors\Error $e)
            {
                ...
                <?php
                error_log('Please check Api Key.');
                return;
            }
            catch (\Exception $e)
            {
        ...
        public function admin_options()
        {
            ...
            echo '<p>'.__('First <a href="https://dashboard.durianpay.id/#/login" target="_blank">login</a> if you have an existing account.'). '</p>';
...
EOT;
        ...
        /**
         * Fetching version info for woo-durianpay and woo-durianpay-subscription
        ...
        public function getVersionMetaInfo($data = array())
        {
            if (isset($data['subscription_id']) && isset($data['recurring'])) {
                ...
                return array(
                    ...
                    'integration_woo_razorpay_version' => get_plugin_data(plugin_dir_path(__FILE__) . 'woo-durianpay.php')['Version'],
                ...
                return array(
                    ...
                    'integration_version' => get_plugin_data(plugin_dir_path(__FILE__) . 'woo-durianpay.php')['Version'],
                    'integration_parent_version' => WOOCOMMERCE_VERSION,
                    'integration_type' => 'plugin',
                );
            }
